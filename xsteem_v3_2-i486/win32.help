...
Steem is freeware, you don't have to pay anything or do anything to entitle you to use the program. However if you like it, or if you don't, you can e-mail us your thoughts. See our website for details (http://steem.atari.st/email.htm).
...
It should be the same, but we might have updated it if many people are having the same problem. If your problem isn't covered then contact us with a full description and we'll try to sort it out if we can.
...
On Linux, there is a fullscreen icon on the bar.  This will switch to fullscreen mode whenever Steem is running if it is turned on. On Windows, you need to click the maximize box on Steem's window (and have DirectX installed). When in fullscreen, the Pause/Break key won't release the mouse without stopping emulation too. You can return to windowed mode by clicking the button in the extreme top-right, or quit by clicking the button in the bottom-left.
...
On the far right is the button to open the disk manager. This window controls all the disks on the ST, and the two boxes up the top hold the disks currently in the drives. 

.Disk Images
Disk images are floppy disks turned into files, this is how most programs are accessed on ST emulators. For details of how to create disk images from ST floppy disks have a look at the "disk image howto.txt" that comes with Steem. 

The large box at the bottom of the disk manager window shows all the disk images in the current directory. The disk manager is like a little Explorer window, you can go into a folder by double clicking on it or by selecting it and pressing return. All disk image files, zip/rar archive files and any Windows shortcuts to those types of files are shown. 

To insert a disk image into a drive, drag it to the box to the right of the big drive icon. To remove it from the drive just drag it back out to the directory view. If you right click in the directory view a menu will pop up allowing you to create a new standard size disk image, a custom size disk image or a folder. Custom disk images can be up to 2 megabytes in size, although larger ones are emulated correctly most TOS versions didn't support them, you may find they will only work with TOS 2.06 or maybe not at all.

Right clicking on a disk image displays a menu with a myriad of options to allow you to organise and use your disk images. "Get Contents" is useful if the disk image is a compilation disk, selecting it will display a list of what the disk contains. "MSA Converter" is a great program that allows you to edit the contents of a disk and convert it to a different format. "Properties" allows you to change the way Steem uses the disk, it is very unlikely you will have to change the setting from the default.
...
Next to disk manager options is the disk image management tools button, it brings up a menu with the following options:

.Search Disk Image Database
Steem uses the great disk image database created and maintained by Keili to be able to recognise disk images. Using this option you can search for a program to find out what disk images contain it. To download the disks have a look at Steem's comprehensive links in the General Info dialog.
...
Clicking on this option brings up a dialog box that allows you to create shortcuts from WinSTon's favourites system. First select the folder containing WinSTon that has the favourites you want to import. Next select the folder that contains WinSTon's disk images (usually called discs). Now select a folder to import the favourites to. The option Only Downloaded Disks when checked causes Steem to create shortcuts only to disks that already exist, when unchecked Steem could end up creating thousands of broken shortcuts which will become unbroken if you download their target disk using WinSTon. The last option tells Steem what to do if the shortcut already exists at the required location, if you set this to skip then it would be quite quick to import the favourites again if you download a new disk through WinSTon you want to use in Steem (thus avoiding the bind of creating loads of broken shortcuts).
...
The next option allows you to select one of three configurations for the ST joysticks, this is useful if you want to switch between different setups quickly.
...
.ST CPU speed
This option can boost the speed of the emulated ST, to improve those painfully slow games that were released. The ST's CPU ran at 8 megahertz (8,000,000 clock cycles per second) but here you can whack it up to get things going faster. A couple of points: setting the CPU speed above 8MHz will cause the emulator to run _slower_, because it has more processing to do for each screen displayed. It will also cause many ST programs to crash/not work properly, because they rely on the processor running at the correct speed.

.Memory size
Here you can select how much memory will be available in the virtual ST. Please note that 14Mb wasn't supported on the ST range of computers without special hacks, so not everything will work with that option selected. Also 4Mb or greater does not work with TOS 1.00. The most compatible choice is 1Mb.
...
Here you can choose what version of the ST operating system you want to use. Displayed in the box are all valid TOS images found in your Steem directory. You have to do a cold reset of the ST before any changes to this option take effect. 
...
.Parallel Port (LPT)
This allows you to connect an ST port to one of your PC's parallel ports. The only option is to choose which one. Warning: This may not work on all versions of Windows. See the FAQ for information on printing from Steem.
...
.Input speed
This option allows you to slow down the speed Steem feeds any received data from the MIDI device to the ST. Due to restrictions of Windows Steem sends all data as fast as possible to the ST regardless of the speed it was actually sent by the device, but some ST programs can't handle that. If you have problems with receiving bank dumps it might be an idea if you slow this down a bit. Also related to this is the shortcut action "Pause until next SysEx", this can get round time-outs on ST programs waiting for a bank dump to start. See the shortcuts section for details of how to assign this action to a key/combination of keys.

o MACROS

This is a handy feature that will allow you to record sequences of input in to the ST and play them back later. At the top there is a display of your macro folder: each macro is stored in a file with the extension stmac, and they are displayed here (if you have any). To record a macro, first click on "New Macro", name it something snazzy and then click on the big red record button. Now go to the main Steem window, start emulation and do some sort of input (move the mouse, press a key, move a joystick). When you have recorded what you want to go back to the macro display and click on the record button again to turn it off. Don't worry about doing this too quickly, Steem doesn't start recording until some input is received and it will cut off any time on the end that doesn't have any input. Now you can replay the actions you recorded by pressing the green play button (next to the record button). 

You can have as many macro files as space on your hard drive will allow. This might mean they get a bit difficult to find on the list, so you can organise them into folders using the macro folder display. Just right click to create a new folder and drag appropriate macros in there. You can also right click on the bold "Macros" folder and choose to open it in Windows explorer if you prefer to use that for your organising.

There are 2 options that affect how macros work:

.Maximum mouse speed
This affects recording and playing macros. Some ST programs won't accept really fast mouse movements as ST mice couldn't generally move very far in a short time. This option lets you allow for this problem by configuring the maximum speed for each macro individually.

.Playback event delay
For most macros you will want your actions to be played back as quickly as possible (for instance in the case of clicking on a shoot button in a game). This option allows you to configure how long Steem will wait between input changes, as some programs won't accept lots of very quick inputs. If speed isn't an issue then you can always set this to "As Recorded", then playing the macro will replay the recorded events at the speed you recorded them at.
...
This option allows you to send certain key combinations that are used by Windows to the ST instead. When it is checked Alt+Tab, Ctrl+Esc and Ctrl+Alt+Delete will go to the ST instead of to the PC. Although Steem is very stable this option could cause a bit of a disaster if Steem stops responding for any reason, you won't be able to shut it down with Ctrl+Alt+Delete, so for safety reasons it is best only to use this option when you really need it. Unfortunately this option doesn't always work in fullscreen mode. Depending on your version of Windows, it may not be possible to capture certain key combinations - you can use shortcuts (see below) to trigger the keys you want from other keys. 
...
This option is not available on Windows NT/XP.  It is a bit of fun really, if you haven't got a sound card you can make your internal PC speaker output Steem's sound. Try it with a few tunes!
...
.Window size
These options allow you to configure exactly the size of Steem's window in the various ST resolutions. First is the option of whether Steem should automatically resize its window when the ST resolution changes or when borders turn on or off. Below that you can set the sizes that Steem should use. With some sizes you have the choice between "Stretch", "No Stretch" and "Grille". "Stretch" makes Steem use the video card to convert the image from the ST output size to the size of the window. This is generally the quickest option, though some video cards can blur ("alpha blit") the stretched image, an effect which some people don't like. "No Stretch" will make Steem draw the ST screen larger in order to avoid stretching. "Grille" makes Steem skip every other line; this is quicker than "No Stretch" but looks pretty ugly. WARNING: On some video cards making the window size bigger than the real size (the first option) will slow down emulation considerably.
...
.CPU speed indicator
This icon appears next to the speed bar when you have ST CPU Speed set to more than 8 megahertz.
...
.Drawing mode
This is a bit of a confusing option, you really don't need to know what the possible settings mean, but which is best will be different depending on your computer's hardware, try them all to see which is quickest. "Stretch blit" makes Steem draw small and then use the video card to stretch the resulting image to fill the screen. This can be slow on very old, bad video cards, and on many video cards it causes the image to be slightly blurred (alpha-blitted), which makes it better resemble the bad televisions usually plugged into STs.  If this blurring annoys you, switch to one of the other modes.
...
.Synchronisation
When the PC and ST are displaying a different number of frames per second you can get jerkiness and wobbling that didn't appear on a real ST. Using these options you can hopefully put that right. Vsync to PC display tells Steem to wait for the PC to be ready before it displays its next frame. To make this option work better you should try and make the PC refresh rate the same or double the ST one. The ST used 50Hz (PAL), 60Hz (NTSC) and 70Hz (Mono).  Many video cards and monitors will refuse to switch to frequencies below 70Hz.
...
If you have DirectX errors when you boot up, or DirectX doesn't work properly, then you can tell Steem to not use it. Sound doesn't work without DirectSound and drawing is slower without DirectDraw.  If you are having problems, it is best to install the latest version of DirectX through Windows Update.
...
In the top left is a display of your shortcuts folder, this contains all your shortcut files (extension stcut). You can select which shortcuts files you want to use by clicking on the icon of the file: a green tick means they are active, a red cross means they are not active. If you have read about macros and profiles in the Options box then you'll know how to create and organise the files. To the right of the folder display is a large box that displays some instructions for people who haven't read the readme. 

Next in the dialog is the shortcuts display, which shows the shortcuts that are in the currently selected file. If you select the default shortcut file (there should be one in there) you can see how shortcuts work. 

For each shortcut there are 3 input pickers (see joysticks section for a description of how they work) separated by "+", this shows you what combination of input will trigger the action (i.e. A + B + C will only perform the action if you press the A, B and C keys simultaneously). If an input picker is blank then it will be ignored, to clear an input picker select it and press Pause/Break.

Next there is "=" followed by a large box, this contains a list of all the possible actions. Most of these are straightforward but a few need more information (these are all followed by "->"):
...
o The Disk Manager is very flexible, using it properly can really help you organise your disk images. The best way is to store all the disk image files in a single directory and then create shortcuts with names that reflect the contents. For instance if you had a compilation disk called Superdooper 1 containing Captive and Bombjack then you would move the disk image to your disk image folder and then make two shortcuts, one called "Captive (SD 1)" and another called "Bombjack (SD 1)". If you do this for all the disks you have then you will end up with a large list of programs at your fingertips, just double click on the one you want to run it. You can also organise them into folders and search for programs by pressing F3. Using the disk image database, you can create shortcuts automatically for every game on a recognised disk.

o It is very easy to transport Steem between different computers without losing all your settings. You need the files Steem.exe, steem.ini and your shortcuts folder to be able to run Steem with your configuration intact. All you need then is a TOS version that you will have to select the first time you run Steem on the new computer. You might find it handy to have unzipd32.dll too, then you can use zipped disk images. Memory snapshots are also transferable, you will have to find the TOS and any disk images that were being used when it was saved, but once you have done that they will load fine.
...
.st, .msa, .stt, .dim, .zip, .rar, .stz, .stx file - load disk & go. If two disks are passed then the first will go in drive A and the second will be in drive B.
...
STFMBORDER - since version 3 Steem has tightened up its border timings to make them more like a real STE, which has stopped some demos that use overscans from working properly. This switch makes Steem emulate border timings like an STFM, allowing you to view the screens.
...
We are very interested in people's comments about Steem. If you have any or you have ideas for how we can improve it please contact us. See our website for details.

---- EMULATION BUG REPORTS ----

If you find an emulation bug please send us an e-mail. It helps if you include the name of the program with the bug and preferably where we can download it. Also the TOS image version number/country name, the memory size and your ST monitor settings when the problem occurs. Please don't send any big attachments (anything over 200Kb) without asking us first.

---- STEEM CRASH REPORTS ----

If Steem crashes or freezes up on you then we want to hear! It is very important to us that Steem is stable. Please send us an e-mail with as much info as you can, tell us what you were doing when it crashed/froze, if it was a one-off or a persistent error, and anything else you think might be appropriate. NOTE: You don't need to include all the technical information that appears in the Windows crash box, the name of the error might be useful but the rest doesn't help at all.


Thanks for reading this file, or just scrolling to the bottom, I hope you enjoy using Steem. Remember to check http://steem.atari.st/ regularly, updates are coming thick and fast! If you don't want to check too often then you can join the Steem update announcement list to get an e-mail whenever a new version is released, go to the Downloads page of the Steem web site for details of how to join.
